...
export const hyperParameterTuning = `!pip install keras-tuner -q

import kerastuner as kt
from tensorflow import keras
from tensorflow.keras import layers

# basic model for tuner
...
# Look at search space
tuner.search_space_summary()

# Preprocess data and search (train)
...
# Typically, to find the model with the best epoch, we use a high value for patience
...
def get_best_trained_model(hp):
    ...
    model.fit(
        ...
        batch_size=128, epochs=int(best_epoch * 1.2)) # since we are training using the full training dataset (including val set) which is about 20% more than the sliced training dataset
    return model
...
# Alternatively, if I want to sacrifice some accuracy for time, I could just get the best model from the search space:
